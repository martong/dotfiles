...
# We are doing a filesystem based search, but it would be nicer, if
# the compilation db's file list could be used. However currently ycm does
# not have code for that.
def findFirstFileWhichIncludesThisHeader(headerFileBasename):
  fileMatches = []
  for root, dirnames, filenames in os.walk(DirectoryOfThisScript()):
  ...
  for fileMatch in fileMatches:
    file = open(fileMatch, "r")
    regex = re.compile(r'include.*' + headerFileBasename)
    for line in file:
      lineMatches = regex.findall(line)
      if (len(lineMatches)):
        return fileMatch
  return None
...
def GetCompilationInfoForFile( filename ):
  ...
  if IsHeaderFile( filename ):
    ...
    if candidateSrcFile != None:
      print "Matching src file, based on method2: " + candidateSrcFile
      return database.GetCompilationInfoForFile(candidateSrcFile)
